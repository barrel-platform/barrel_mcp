...
basic_legacy_sha256_still_verifies_test() ->
    ...
    Legacy = barrel_mcp_auth_basic:hash_password(
        <<"hunter2">>,
...
basic_iterations_are_overridable_test() ->
    H = barrel_mcp_auth_basic:hash_password(
        <<"x">>,
        #{iterations => 1000}
    ),
...
apikey_hmac_format_test() ->
    H = barrel_mcp_auth_apikey:hash_key(
        <<"my-key">>,
...
apikey_hmac_is_pepper_dependent_test() ->
    H1 = barrel_mcp_auth_apikey:hash_key(
        <<"my-key">>,
        #{pepper => <<"a">>}
    ),
    H2 = barrel_mcp_auth_apikey:hash_key(
        <<"my-key">>,
        #{pepper => <<"b">>}
    ),
    ?assertNotEqual(H1, H2).
